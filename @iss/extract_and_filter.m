    ...
    for r = 1:o.nRounds+o.nExtraRounds
        ...
        for t = 1:nSerieswPos  
            ...
            if (r ~=o.AnchorRound && strcmpi(o.ExtractScale, 'auto')) ||...
                ...
                if t>1 || ismember(r,setdiff(2:o.nRounds+o.nExtraRounds,o.AnchorRound))
                    if r==o.AnchorRound
                        error(['Some tiles in imaging rounds already exist, but o.ExtractScaleAnchor = auto.'...
                            ...
                            'delete tile 1 in round %d and rerun, tile location:\n%s.'],o.AnchorRound,fName{1});
                    else
                        ...
                        error(['Some tiles in imaging rounds already exist, but o.ExtractScale = auto.'...
                            ...
                            '\nIf tiles up to this point were obtained with auto value,'...
                            ' delete tile 1 in round %d and rerun, tile location:\n%s.'],...
                            r0,fullfile(o.TileDirectory,[o.FileBase{r0}, '_t', num2str(1), '.tif']));
                    end
                end
